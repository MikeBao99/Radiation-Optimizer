...
var y >=0;
var max_buf {ROWS, COLUMNS} >=0;
var min_buf {ROWS, COLUMNS} >= 0;				   # index across all matrices given
...
subject to Allowed {i in ROWS, j in COLUMNS}: (all_dose + min_buf[i,j]) >= crit[i,j] * ((1-nabla) * (sum {k in MATS} I[k]*beam_dose[k,i,j]) + nabla*(sum {k in MATS, i2 in (i-1)..(i+1), j2 in (j-1)..(j+1)} I[k]*beam_dose[k,i2,j2]));